//! Generated compact Unicode 16.0.0 General_Category=L lookup.
//! Regenerate from Python unicodedata when updating the Unicode baseline.
...
test "Unicode letter lookup separates letters from digits symbols and emoji" {
    try std.testing.expect(isLetter('A'));
    try std.testing.expect(isLetter(0x00E9));
    try std.testing.expect(isLetter(0x754C));
    try std.testing.expect(!isLetter('0'));
    try std.testing.expect(!isLetter(0x0661));
